...
const updateMetaballs = (metaballs, width, height, isMobile) => {
  ...
  metaballs.forEach((ball, i) => {
    ...
    if (ball.formation) {
      ball.vx += (ball.targetX - ball.x) * SETTINGS.formationAttractionFactor - SETTINGS.damping * ball.vx;
      ball.vy += (ball.targetY - ball.y) * SETTINGS.formationAttractionFactor - SETTINGS.damping * ball.vy;
    }
    ...
    const handleMouseDown = (e) => {
      ...
      formationTimer = setTimeout(async () => {
        ...
        if (svgPaths.length > 0) {
          const pathData = svgPaths[Math.floor(Math.random() * svgPaths.length)]; // Випадковий шлях
